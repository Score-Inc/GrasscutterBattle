...
public class Scene {
    ...
    public void setDungeonData(DungeonData dungeonData) {
        if (dungeonData == null || this.dungeonData != null || this.getSceneType() != SceneType.SCENE_DUNGEON
    ...
    private synchronized void removePlayerAvatars(Player player) {
        ...
        // removeEntities(team, VisionType.VISION_TYPE_REMOVE); // List<SubType> isn't
        // cool apparently :(
    ...
    public synchronized void replaceEntity(EntityAvatar oldEntity, EntityAvatar newEntity) {
        ...
        this.broadcastPacket(
                new PacketSceneEntityAppearNotify(newEntity, VisionType.VISION_TYPE_REPLACE, oldEntity.getId()));
    }

    public void showOtherEntities(Player player) {
        GameEntity currentEntity = player.getTeamManager().getCurrentAvatarEntity();
        List<GameEntity> entities = this.getEntities().values().stream().filter(entity -> entity != currentEntity)
                .toList();
    ...
    public void handleAttack(AttackResult result) {
        ...
        // TODO: Add Better Translation

        // 怪物攻击 - Monster Attack
        if (attacker instanceof EntityMonster) {
            // 怪物 - Monster
            if (target instanceof EntityMonster) {
                // 宠物误伤自己 或 者其他怪物攻击了宠物 - Send log to console if the pet accidentally injured
                // itself or another monster attacked the pet
                if (Grasscutter.playerPetMap.get(target.getId()) != null) {
                    /*
                     * Grasscutter.getLogger()
                     * .info("怪物 " + String.valueOf(attacker.getId()) + " 攻击了宠物 " +
                     * String.valueOf(target.getId())
                     * + " 伤害为 " + String.valueOf(result.getDamage()) + "! (宠物不受任何的伤害)");
                     */
                    Grasscutter.getLogger()
                            .info("Monster: " + String.valueOf(attacker.getId()) + " Attacked a pet, with ID: "
                                    + String.valueOf(target.getId())
                                    + " the output damage was: " + String.valueOf(result.getDamage())
                                    + "! (The pet does not take any damage)");
                    return;
                }
                // Grasscutter.getLogger().info("怪物 " + String.valueOf(attacker.getId()) + "
                // 攻击怪物 " + String.valueOf(target.getId()) + " 伤害为 " +
                // String.valueOf(result.getDamage()) );
                Grasscutter.getLogger()
                        .info("Monster: " + String.valueOf(attacker.getId()) + " Attacked a pet, with ID: "
                                + String.valueOf(target.getId()) + " the output damage was: "
                                + String.valueOf(result.getDamage()));
                ...
                // Grasscutter.getLogger().info("怪物 " + String.valueOf(attacker.getId()) + "
                // 攻击玩家 " + String.valueOf(player.getUid()) + " : (" +
                // String.valueOf(attacker.getId()) +")" + " 伤害为 " +
                // String.valueOf(result.getDamage()) );
                Grasscutter.getLogger()
            ...
            Player player = temp.getPlayer();
            Grasscutter.playerGameEntityMap.put(player.getUid(), attacker);
            // 玩家攻击的怪物 - send log if monster is attacked by the player
            if (target instanceof EntityMonster) {
                // 是宠物 - is a pet(?)
                if (Grasscutter.playerPetMap.get(target.getId()) != null) {
                    // 扫描场景 - detect the scene
                    for (GameEntity gameEntity : player.getScene().getEntities().values()) {
                        if (gameEntity instanceof EntityAvatar) {
                            // 场景中的实体属于玩家 - if the entity in the scene belongs to the player
                            EntityAvatar avatarTemp = (EntityAvatar) gameEntity;
                            Player playerTemp = avatarTemp.getPlayer();
                            Grasscutter.playerGameEntityMap.put(playerTemp.getUid(), gameEntity);
                        }
                    }
                    ...
                    for (GameEntity gameEntity : Grasscutter.playerGameEntityMap.values()) {
                        if (gameEntity instanceof EntityAvatar) {
                            // 场景中的实体属于玩家 - if the entity in the scene belongs to other player
                            EntityAvatar avatarTemp = (EntityAvatar) gameEntity;
                            Player playerTemp = avatarTemp.getPlayer();
                            // Grasscutter.getLogger().info(" 玩家 " + String.valueOf(playerTemp.getUid()) + "
                            // : (" + String.valueOf(gameEntity.getId()) + ")" );
                            Grasscutter.getLogger()
                                    .info("Owner UID: " + String.valueOf(playerTemp.getUid()) + " Pet ID: ("
                                            + String.valueOf(gameEntity.getId()) + ")");

                        }
                    }
                    if (Grasscutter.playerPetMap.get(target.getId()) == player.getUid()) {
                        // 攻击自己的宠物 - log if attacked your own pet
                        // Grasscutter.getLogger().info("玩家 " + String.valueOf(player.getUid()) + "
                        // 攻击了自己的宠物 " + String.valueOf(target.getId()) + " 伤害为 " +
                        // String.valueOf(result.getDamage()) + " 攻击无效!(不能攻击自己的宠物)" );
                        Grasscutter.getLogger()
                                .info("Player UID: " + String.valueOf(player.getUid())
                                        + " Attacked their own pet, with ID: "
                                        + String.valueOf(target.getId()) + " the output damage was: "
                                        + String.valueOf(result.getDamage())
                                        + " the pet does not take any damage!(Cannot attack their own pet)");

                        return;
                    } else {
                        // 攻击别人的宠物 - log if attacked other players pet
                        Integer otherUid = Grasscutter.playerPetMap.get(target.getId());
                        GameEntity otherGameEntity = Grasscutter.playerGameEntityMap.get(otherUid);
                        if (otherGameEntity == null) {
                            return;
                        }
                        target = otherGameEntity;
                        // Grasscutter.getLogger().info("玩家 " + String.valueOf(player.getUid()) + " : ("
                        // + String.valueOf(attacker.getId()) +")" + " 攻击了玩家 " +
                    ...
                    // 怪物
                    // Grasscutter.getLogger().info("玩家 " + String.valueOf(player.getUid()) + " : ("
                    // + String.valueOf(attacker.getId()) +")" + " 攻击了怪物 " +
                    // String.valueOf(target.getId()) + " 伤害为 " + String.valueOf(result.getDamage())
                    // );
                    Grasscutter.getLogger()
                ...
                // Grasscutter.getLogger().info("玩家 " + String.valueOf(player.getUid()) + " : ("
                // + String.valueOf(attacker.getId()) +")" + " 攻击了玩家 " +
                // String.valueOf(player2.getUid()) + " 伤害为 " +
                // String.valueOf(result.getDamage()) );
                Grasscutter.getLogger()
                        .info("Player UID: " + String.valueOf(player.getUid()) + " with pet ID: "
                                + String.valueOf(attacker.getId())
                                + " attacked Player UID: " + String.valueOf(player2.getUid())
                                + " the output damage was: "
                                + String.valueOf(result.getDamage()));
        ...
        if (attacker != null) {
            ...
            if (attacker instanceof EntityClientGadget gadgetAttacker) {
                ...
                if (clientGadgetOwner instanceof EntityAvatar) {
                    ((EntityClientGadget) attacker).getOwner().getCodex().checkAnimal(target,
    ...
    public synchronized void checkSpawns() {
        ...
        for (Player player : this.getPlayers()) {
            for (SpawnDataEntry.GridBlockId block : SpawnDataEntry.GridBlockId
                    .getAdjacentGridBlockIds(player.getSceneId(), player.getPosition()))
                loadedGridBlocks.add(block);
        }
        if (this.loadedGridBlocks.containsAll(loadedGridBlocks)) { // Don't recalculate static spawns if nothing has
                                                                   // changed
            return;
        }
        ...
        for (SpawnDataEntry entry : visible) {
            // If spawn entry is in our view and hasnt been spawned/killed yet, we should
            // spawn it
    ...
    public List<SceneGroup> playerMeetGroups(Player player, SceneBlock block) {
        List<SceneGroup> sceneGroups = SceneIndexManager.queryNeighbors(block.sceneGroupIndex,
    ...
    public void loadTriggerFromGroup(SceneGroup group, String triggerName) {
        ...
        getScriptManager()
                .registerTrigger(group.triggers.values().stream().filter(p -> p.name.contains(triggerName)).toList());
    ...
    public void onLoadGroup(List<SceneGroup> groups) {
        ...
        for (SceneGroup group : groups) {
            ...
            // just load the 'init' suite, avoid spawn the suite added by AddExtraGroupSuite
            // etc.
        ...
        // groups.forEach(g -> scriptManager.callEvent(EventType.EVENT_GROUP_LOAD,
        // null));
    ...
    public void addItemEntity(int itemId, int amount, GameEntity bornForm) {
        ...
        if (itemData.isEquip()) {
            ...
            EntityItem entity = new EntityItem(this, null, itemData, bornForm.getPosition().clone().addZ(.9f), amount); // Why
                                                                                                                        // Z?
            addEntity(entity);
        }
    }
